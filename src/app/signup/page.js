...
const Signup = () => {
    ...
    const handelOnSubmit = async (e) => {
        ...
        try {
            ...
            // console.log("respons in submit", response)
            toast.success("Account creates suueccfully")
            router.push('/login')
        } catch (error) {
    ...
    useEffect(() => {
        if (user.email.length > 0 && user.password.length > 0 && user.username > 0) {
        ...
        <>
            ...
            <Container className="mt-5">
                <Card style={{ width: '35rem' }} className="mx-auto p-5">
                    <h3 className="text-center">{loading ? "Proceesing" : "Signup"}</h3>
                    <Form>
                        ...
                        <Button variant="secondary" type="submit" onClick={handelOnSubmit} className="mt-4">
                            {buttonDisabeled ? "Please Fill The form" : "Signup"} Submit
                        </Button>

                    </Form>
                    <Link href="/login" className="mt-4" style={{ textDecoration: "none" }}> visit Login</Link>
                </Card>
            </Container>
        </>
    )
